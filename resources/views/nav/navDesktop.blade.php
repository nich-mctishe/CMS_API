<nav class="nav desktop" data-ng-controller="NavController" data-ng-class="'initial'">
    <div class="mainCircle">
        <a href="#/">
            <h1>Nicholas</h1>
            <h1>Headlong</h1>
            <h2>INTERACTIVE DEVELOPER</h2>
        </a>
    </div>
    <ul class="mainMenu" data-ng-class="'initial'">
        <li><a href="#projects">Projects</a></li>
        <li><a href="#projlets">Projlets</a></li>
        <li><a href="#skills">Skills</a></li>
        <li><a href="#work-experience" class="double-line">Work Experience</a></li>
        <li><a href="http://blog.nocholasheadlong.co.uk">Blog</a></li>
        <li><a href="/downloads/nicholasheadlongCv.pdf">CV</a></li>
        <li><a href="#contact">Contact</a></li>
    </ul>
</nav>